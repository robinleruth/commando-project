...
class MovingAverageStrategyService(StrategyService):
    ...
    def compute(self):
        ...
        self.df['short_term_trend'] = np.round(self.df[app_config.CLOSE].rolling(self.short_term_trend).mean())
        self.df['long_term_trend'] = np.round(self.df[app_config.CLOSE].rolling(self.long_term_trend).mean())

        self.df['short_term_trend - long_term_trend'] = self.df['short_term_trend'] - self.df['long_term_trend']

        self.df['Regime'] = np.where(self.df['short_term_trend - long_term_trend'] > signal_threshold, 1, 0)
        self.df['Regime'] = np.where(self.df['short_term_trend - long_term_trend'] < -signal_threshold, -1, self.df['Regime'])
